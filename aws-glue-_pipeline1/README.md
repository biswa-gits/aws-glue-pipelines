...
```bash
aws iam create-role \
  --role-name _pipeline1-glue-role \
  --assume-role-policy-document file://iam/glue_trust_policy.json

aws iam put-role-policy \
  --role-name _pipeline1-glue-role \
  --policy-name _pipeline1-glue-permissions \
...
aws cloudformation deploy \
  --template-file cloudformation/_pipeline1-glue-job.yaml \
  --stack-name _pipeline1-stack \
...
### 5. Run the Glue Job
```bash
aws glue start-job-run --job-name _pipeline1-dev
```
