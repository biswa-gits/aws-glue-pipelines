"""
AWS Glue ETL Job: pl_ingestion_customer_table_2
...
def main():
    ...
    # Activity: Copy from SQl to ADLS (Copy: AzureSqlSource -> DelimitedTextSink)
    source_df = glueContext.create_dynamic_frame.from_options(
        connection_type="s3",
        format="csv",
        connection_options={"paths": [SOURCE_PATH]},
        transformation_ctx="Copy from SQl to ADLS_source"
    )
    ...
    sink_df = glueContext.write_dynamic_frame.from_options(
        ...
        transformation_ctx="Copy from SQl to ADLS_sink"
    )
