"""
AWS Glue ETL Job: pl_ingest_product_sqldb_to_adls_2
...
def main():
    ...
    # Activity: Product SQLDB to ADLS (Copy: AzureSqlSource -> JsonSink)
    source_df = glueContext.create_dynamic_frame.from_options(
        connection_type="s3",
        format="csv",
        connection_options={"paths": [SOURCE_PATH]},
        transformation_ctx="Product SQLDB to ADLS_source"
    )
    ...
    sink_df = glueContext.write_dynamic_frame.from_options(
        ...
        transformation_ctx="Product SQLDB to ADLS_sink"
    )
