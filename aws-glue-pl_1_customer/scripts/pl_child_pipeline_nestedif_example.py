"""
AWS Glue ETL Job: pl_child_pipeline_nestedif_example
...
def main():
    ...
    # Activity: Get Customer Count (Lookup)
    lookup_df = glueContext.create_dynamic_frame.from_catalog(
        database="default",
        table_name="lookup_table",
        transformation_ctx="Get Customer Count_lookup"
    )

    # Activity: If Condition1 (IfCondition)
    # Original expression: @greater(activity('Get Customer Count').output.firstRow.RecordCount,500)
