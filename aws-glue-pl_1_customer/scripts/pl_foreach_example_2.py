"""
AWS Glue ETL Job: pl_foreach_example_2
...
def main():
    ...
    # Activity: ACT_Lookup_table (Lookup)
    lookup_df = glueContext.create_dynamic_frame.from_catalog(
        database="default",
        table_name="lookup_table",
        transformation_ctx="ACT_Lookup_table_lookup"
    )

    # Activity: If Condition1 (IfCondition)
    # Original expression: @greater(activity('ACT_Lookup_table').output.firstRow.RecordCount,100)
