"""
AWS Glue ETL Job: pl_ingestion_product_addres_to_json_child
...
def main():
    ...
    # Activity: ACT_Get_Address_Count (Lookup)
    lookup_df = glueContext.create_dynamic_frame.from_catalog(
        database="default",
        table_name="lookup_table",
        transformation_ctx="ACT_Get_Address_Count_lookup"
    )

    # Activity: ACT_Check_Count (IfCondition)
    # Original expression: @greater(activity('ACT_Get_Address_Count').output.firstRow.recordCount,100)
