"""
AWS Glue ETL Job: pl_ingestion_customer_sqldb_adls
...
def main():
    ...
    # Activity: Ingestion_Customer_SQLDB_ADLS (Copy: AzureSqlSource -> DelimitedTextSink)
    source_df = glueContext.create_dynamic_frame.from_options(
        connection_type="s3",
        format="csv",
        connection_options={"paths": [SOURCE_PATH]},
        transformation_ctx="Ingestion_Customer_SQLDB_ADLS_source"
    )
    ...
    sink_df = glueContext.write_dynamic_frame.from_options(
        ...
        transformation_ctx="Ingestion_Customer_SQLDB_ADLS_sink"
    )
