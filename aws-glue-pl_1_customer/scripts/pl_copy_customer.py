"""
AWS Glue ETL Job: pl_copy_customer
...
entityName = ""  # Parameter: string


def main():

    # Activity: Copy data1 (Copy: AzureSqlSource -> DelimitedTextSink)
    source_df = glueContext.create_dynamic_frame.from_options(
        connection_type="s3",
        format="csv",
        connection_options={"paths": [SOURCE_PATH]},
        transformation_ctx="Copy data1_source"
    )
    ...
    sink_df = glueContext.write_dynamic_frame.from_options(
        ...
        transformation_ctx="Copy data1_sink"
    )
